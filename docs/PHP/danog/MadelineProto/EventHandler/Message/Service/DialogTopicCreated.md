...
## Method list:
* [`__construct(\danog\MadelineProto\MTProto $API, array $rawMessage, array $info, string $title, int $iconColor, ?int $iconEmojiId)`](#__construct)
...
## Methods:
### <a name="__construct"></a> `__construct(\danog\MadelineProto\MTProto $API, array $rawMessage, array $info, string $title, int $iconColor, ?int $iconEmojiId)`
