---
title: "danog\\MadelineProto\\EventHandler\\Message\\Service\\DialogChatJoinedByLink: A user joined the chat via an invite link."
description: ""
...
## Properties
* `$inviterId`: `int` ID of the user that created the invite link
* `$id`: `int` Message ID
...
## Method list:
* [`__construct(\danog\MadelineProto\MTProto $API, array $rawMessage, array $info, int $inviterId)`](#__construct-danog-madelineproto-mtproto-api-array-rawmessage-array-info-int-inviterid)
...
## Methods:
### `__construct(\danog\MadelineProto\MTProto $API, array $rawMessage, array $info, int $inviterId)`
