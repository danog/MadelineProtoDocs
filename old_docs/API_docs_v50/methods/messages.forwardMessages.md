...
$Updates = $MadelineProto->messages->forwardMessages(['silent' => Bool, 'background' => Bool, 'with_my_score' => Bool, 'drop_author' => Bool, 'drop_media_captions' => Bool, 'from_peer' => InputPeer, 'id' => [int, int], 'to_peer' => InputPeer, 'schedule_date' => int, ]);
```

Or, if you're into Lua:

```lua
Updates = messages.forwardMessages({silent=Bool, background=Bool, with_my_score=Bool, drop_author=Bool, drop_media_captions=Bool, from_peer=InputPeer, id={int}, to_peer=InputPeer, schedule_date=int, })
```
